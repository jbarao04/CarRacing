...
class CarRacing(gym.Env, EzPickle):
    ...
    * `continuous=True` converts the environment to use discrete action space.
    ...
    def _create_track(self):
        ...
        self.np_random.shuffle(possible_points)
        for i in range(5):
            x1, y1 = possible_points[i]
            self.obstacles.append((x1, y1))
            square_vertices = [
            ...
            self.road_poly.append((square_vertices, (204, 196, 186)))  # Black square
        self.track = track
        return True
    ...
    def step(self, action: Union[np.ndarray, int]):
        ...
        if action is not None:  # First step without action, called from reset()
            ...
            for i in range(5):
                x1, y1 = self.obstacles[i]
                half_len = SIDE_LENGTH
